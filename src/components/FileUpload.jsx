...
import {
  ...
  MessageSquare, ChevronRight, Eye, Table2,
  Hash, Type, ChevronDown
} from 'lucide-react'
...
export default function FileUpload({ onNavigateToChat }) {
  ...
  const processFiles = (incoming) => {
    const newFiles = Array.from(incoming).map((file) => ({
      ...
      size:   file.size,
      valid:  isValid(file),
      status: isValid(file) ? 'ready' : 'error',
    }))
      ...
      if (ext === 'csv') {
        reader.onload = (e) => {
          try {
            ...
            resolve(buildDataset(fileObj, ext, parsed))
          } catch (err) {
        ...
        // Excel — use ArrayBuffer + xlsx package
        reader.onload = async (e) => {
          try {
            const parsed = await parseExcel(e.target.result)
            resolve(buildDataset(fileObj, ext, parsed))
          } catch (err) {
  ...
  const buildDataset = (fileObj, ext, parsed) => ({
    ...
    rows:     parsed.rows,     // sample (first 10)
    allRows:  parsed.allRows,  // full data for stats
  ...
  // Derive smart insights from parsed data
  ...
  return (
    <div className="max-w-3xl mx-auto space-y-6">
      ...
      <div
        ...
        className={
          ...
          (isDragging
            ...
            : 'border-white/10 hover:border-cyan-500/40 hover:bg-white/2 bg-white/1')
        }
      >
        <input
          ref={inputRef}
        ...
        <div className={'w-20 h-20 rounded-2xl mx-auto mb-6 flex items-center justify-center transition-all duration-300 ' +
          (isDragging ? 'bg-cyan-500/20' : 'glass')}>
        ...
        <div className="flex items-center justify-center gap-2 mt-6 flex-wrap">
          {allowedTypes.map((ext) => (
            <span key={ext} className="px-3 py-1 rounded-full glass border border-white/10 text-xs text-gray-400 uppercase font-medium">
              .{ext}
            </span>
          ))}
        </div>
      ...
      {files.length > 0 && (
        <div className="glass-card rounded-2xl border border-white/5 overflow-hidden">
          <div className="flex items-center justify-between px-6 py-4 border-b border-white/5">
            ...
            <button onClick={handleReset} className="text-xs text-gray-500 hover:text-rose-400 transition-colors">
              Clear all
            </button>
          </div>
                ...
                <div key={f.id} className="flex items-center gap-4 px-6 py-4">
                  ...
                  <div className="flex-1 min-w-0">
                    <div className="flex items-center gap-2 mb-1">
                      ...
                      {!f.valid && (
                        <span className="flex-shrink-0 text-xs px-2 py-0.5 rounded-full bg-rose-500/10 text-rose-400">
                          Invalid
                        </span>
                      )}
                    </div>
          ...
          {!analyzed && (
            <div className="px-6 py-4 border-t border-white/5">
              <button
                ...
                disabled={analyzing || validCount === 0}
                className={
              ...
              >
                {analyzing ? (
                  ...
                  <>
                    <TrendingUp size={16} />
                    Analyze {validCount} file{validCount !== 1 ? 's' : ''} with AI
                  </>
                )}
              </button>
            </div>
        ...
        <div className="glass-card rounded-2xl border border-emerald-500/20 overflow-hidden">
          ...
          {parsedDataset.xlsxMissing && (
            <div className="mx-6 mt-4 flex items-start gap-2 p-3 rounded-xl bg-amber-500/10 border border-amber-500/20">
              ...
              <div>
                <p className="text-xs text-amber-400 font-medium">Excel parsing requires the xlsx package</p>
                <p className="text-xs text-gray-500 mt-0.5">
                  Run in your terminal:{' '}
                  <code className="text-cyan-400 bg-black/30 px-1 py-0.5 rounded">npm install xlsx</code>
          ...
          <div className="px-6 pb-4">
            ...
            <div className="flex flex-wrap gap-1.5">
              ...
              {parsedDataset.columns.length > 12 && (
                <span className="text-xs px-2.5 py-1 rounded-full glass border border-white/10 text-gray-500">
                  +{parsedDataset.columns.length - 12} more
                </span>
              )}
            </div>
          </div>
          ...
          <div className="px-6 pb-6 flex gap-3">
            ...
            >
              <MessageSquare size={15} /> Ask AI About This Data
        ...
        <div className="flex items-start gap-3 p-4 rounded-xl glass border border-amber-500/20">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500 leading-relaxed">
              CSV and JSON files are parsed instantly — no setup needed.
              For Excel (.xlsx/.xls), run <code className="text-cyan-400">npm install xlsx</code> in your project once.
              Max 50MB per file. Your data stays on your machine — never sent anywhere.
            </p>
          </div>
        </div>
      )}
      ...
      {previewOpen && parsedDataset && (
        ...
        >
          <div
            className="glass-card rounded-3xl border border-white/10 w-full max-w-4xl max-h-[85vh] flex flex-col overflow-hidden"
          ...
          >
            ...
            <div className="flex items-center justify-between px-6 py-4 border-b border-white/8">
              <div>
                ...
                <p className="text-xs text-gray-500 mt-0.5">
                  {parsedDataset.rowCount.toLocaleString()} rows · {parsedDataset.columns.length} columns · {parsedDataset.size}
                </p>
              </div>
              <button
                onClick={() => setPreviewOpen(false)}
            ...
            <div className="px-6 py-3 border-b border-white/5 flex flex-wrap gap-1.5">
              {parsedDataset.columns.map((col) => {
                ...
                return (
                  <span
                    key={col}
                    title={isNum ? 'Numeric column' : 'Text column'}
                    className={
              ...
              {parsedDataset.rows.length > 0 ? (
                <>
                  <div className="flex items-center gap-2 mb-3">
                    ...
                    <p className="text-xs text-gray-500">
                      Showing first {parsedDataset.rows.length} of {parsedDataset.rowCount.toLocaleString()} rows
                    </p>
                  </div>
                          ...
                          {parsedDataset.columns.map((col) => (
                            <th key={col} className="text-left text-gray-400 font-medium px-4 py-3 whitespace-nowrap">
                              <div className="flex items-center gap-1.5">
                                {isNumericCol(col, parsedDataset.rows)
                        ...
                        {parsedDataset.rows.map((row, i) => (
                          <tr key={i} className="hover:bg-white/2 transition-colors">
                            ...
                            {parsedDataset.columns.map((col) => (
                              <td
                                key={col}
                                className="px-4 py-2.5 text-gray-300 whitespace-nowrap max-w-[180px] truncate text-sm"
                ...
                <div className="text-center py-12">
                  ...
                  <p className="text-gray-400 text-sm font-medium">No row preview available</p>
                  <p className="text-gray-600 text-xs mt-1">
                    Install <code className="text-cyan-400">xlsx</code> package to parse Excel files
                  </p>
                </div>
              )}
            </div>
            ...
            <div className="px-6 py-4 border-t border-white/5 flex items-center justify-between">
              <p className="text-xs text-gray-600">
                <span className="text-violet-400 font-medium">■</span> Numeric &nbsp;
                <span className="text-cyan-400 font-medium">■</span> Text
              </p>
              <button
                onClick={() => { setPreviewOpen(false); handleAskAI() }}
...
function buildInsights(dataset) {
  ...
  // Row / column count
  insights.push(
    `${dataset.rowCount.toLocaleString()} rows and ${dataset.columns.length} columns detected`
  )

  // Column names
  const colPreview = dataset.columns.slice(0, 6).join(', ')
  insights.push(
    `Columns: ${colPreview}${dataset.columns.length > 6 ? ` and ${dataset.columns.length - 6} more` : ''}`
  )
  ...
  if (numCols.length > 0) {
    ...
    if (vals.length > 0) {
      ...
      insights.push(
        `"${col}" ranges from ${min} to ${max} with an average of ${avg}`
      )
    }
  }
